...
이전까지 고려하지 못했던 기계어로 변하는 과정 ( 전처리 -> 컴파일 -> 어셈블-> 링킹 )을 이해하여,  
버블 정렬 알고리듬의 동작을 cpu에서는 어떻게 이루어지는지 이해하고 서술한다.   

여기서 저는 우리가 만든 logic은 stack 메모리 단에서만 이루어지기 때문에, 이곳을 중점으로 설명하였습니다.  
...
때문에 메모리 언더플로우를 해결하기 위한 방안을 분기로 두어 레포트를 작성하였습니다. 



# 어셈블리어 분석

해당 코드를 asm 변환 사이트를 통해서 risc-v의 표준 명령어셋으로 이루어진 asm 코드로 만들었습니다. 
이것을 순차적으로 살펴보면서 어떻게 동작이 이루어지는지 확인해보겠습니다.
...
저는 기존에 C 코드가 컴파일 과정을 거치면서 stack 영역에서 일어나는 일에 대해서 무지했습니다.  
  
  
효율적인 메모리 사용을 하기 위해 어떻게 코드를 만들어야될까 고민하여 함수 호출을 최소화 하고,
   
   
함수의 크기를 늘리는 한이 있더라도 최대 크기를 줄이는 방향으로 코드를 수정해보았습니다.
...
.L5:
        lw      a5,-24(s0) # a5 = j 
        slli    a5,a5,2    # j<<2
        lw      a4,-28(s0) # a4 = pData(배열포인터) 
        add     a5,a4,a5   # a5 = pData + j<<2
        lw      a4,0(a5)   # a4 = s0
        lw      a5,-24(s0) # a5 = j
        addi    a5,a5,1    # 
        slli    a5,a5,2
...
- 기존: 48바이트
- 인라인화: 64바이트
- 원인: 모든 변수를 하나의 함수에서 관리

### 2. 코드 크기 증가
- 함수 호출 코드 제거
- 하지만 swap 로직이 반복되어 코드 크기 증가


## 결론

인라인화를 통해 함수 호출 오버헤드를 제거하고 전체적인 메모리 사용량을 줄였습니다. 하지만 개별 함수의 스택 크기는 증가했으며, 이는 모든 변수를 하나의 스택 프레임에서 관리하기 때문입니다. 전반적으로 성능과 메모리 효율성이 향상되었습니다.
